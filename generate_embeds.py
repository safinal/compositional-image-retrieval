...
def encode_queries(model, loader) -> np.ndarray:
    """
    Process query pairs and generate embeddings.
    ...
    """
    all_embeddings = []
    for query_images, query_texts, _ in tqdm(loader):
        query_images = query_images.to('cuda')
        with torch.no_grad():
            final_embedding = model(query_images, query_texts)
        all_embeddings.append(final_embedding.detach().cpu().numpy())
    return np.concatenate(all_embeddings)


def encode_database(model, loader) -> np.ndarray :
    """
    Process database images and generate embeddings.
    ...
    """
    all_embeddings = []
    for _, _, target_images in tqdm(loader):
        target_images = target_images.to('cuda')
        with torch.no_grad():
            target_imgs_embedding = model.encode_database_image(target_images)
        all_embeddings.append(target_imgs_embedding.detach().cpu().numpy())
    return np.concatenate(all_embeddings)