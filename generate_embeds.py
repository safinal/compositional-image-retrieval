...
def encode_database(df: pd.DataFrame) -> np.ndarray :
    ...
    all_embeddings = []
    for i in range(0, len(df), batch_size):
        query_imgs = torch.stack([model.processor(Image.open(query_image_path)) for query_image_path in df['target_image'][i:i+batch_size]]).to(device)
        with torch.no_grad():
            query_imgs_embedding = model(query_imgs)
        query_imgs_embedding /= query_imgs_embedding.norm(dim=-1, keepdim=True)
        all_embeddings.append(query_imgs_embedding.detach().cpu().numpy())
    return np.concatenate(all_embeddings)
